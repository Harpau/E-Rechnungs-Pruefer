...
class RootfsBuilder:
    ...
    def fontconfig_link_provenance(self, path: str) -> dict[str, Any]:
        # These exact names are generated by Debian fontconfig-config.postinst
        # 2.15.0-2.3 from Debconf choices; most other conf.d links are packaged.
        allowed = {
            "10-unhinted.conf",
            "10-autohint.conf",
        ...
        if name not in allowed:
            raise RootfsError(f"Unknown generated Fontconfig configuration: {path}")
        package = self.package("fontconfig-config")
        target = "/usr/share/fontconfig/conf.avail/" + name
        template = image_path(self.source, target)
        generator = self.source / "var/lib/dpkg/info/fontconfig-config.postinst"
        if (
            os.readlink(image_path(self.source, path)) != target
            or not template.is_file()
            or template.is_symlink()
            or package not in self.owners.get(target, [])
            or not generator.is_file()
            or generator.is_symlink()
        ):
            raise RootfsError(f"Unbound Fontconfig generated link: {path}")
        fields = email.parser.Parser().parsestr(self.statuses[package])
        self.packages.add(package)
        return {
            "kind": "debian-generated-symlink",
            "packages": [package],
            "target": target,
            "generator": "fontconfig-config postinst",
            "generator_version": fields["Version"],
            "generator_path": "/var/lib/dpkg/info/fontconfig-config.postinst",
            "generator_sha256": sha256(generator),
            "inputs": {target: sha256(template)},
        }
    ...
    def provenance(self, path: str, *, symlink: bool = False) -> dict[str, Any]:
        ...
        if symlink and posixpath.dirname(path) == "/etc/fonts/conf.d":
            return self.fontconfig_link_provenance(path)
        generated = {
    ...
    def runtime_roots(self) -> None:
        ...
        # Retain installed Fontconfig data; application Noto fonts are copied
        # with /app and tested separately.
        ...
        providers = (
            ...
            "libfreebl3.so",
            "libfreeblpriv3.so",
            # OpenJDK's headless fontpath.c opens this by SONAME with dlopen.
            "libfontconfig.so.1",
        )
